...
- **Next:** C (program anchor pins) + D (N-candidate generation), which meet at anchors-in-variants; then E (agent-critic, offline/gated). Also queued: stand up vitest **after B** seeded with the rotate-grip + delete-gizmo regressions (deferred so B's rewrite didn't invalidate them).
