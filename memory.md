...
- **Workstream B DONE (Submit→Results→Editor IA).** Wizard is now Property→Space→Program→**Style**; the Style step's **Submit** is the only generation trigger. App owns the generation job (so Results reflects processing→ready/error). New **Results** destination (`components/Results.tsx`): scored-card grid (inert `compact` PlanCanvas thumbnails), one Recommended pick, per-card Open-in-editor, plus an **Edited designs** list. Editor is an App-level view; **fork-on-first-edit** (SceneEditor `dirty = history.length > 1`; Done saves only if a command committed, so open+Done-with-no-edits leaves no row). Persistence: `generatedAlternatives` on the project (plan once, surfaced only), quota-safe write shared with `saveEditedDesign`; a 3-alternative record ≈ **31 KB** (far under quota). **Removed** (grep-clean): the Review wizard step, the `Resume editing` dashboard side-door, the modal editor entry, VersionList/adopt/PlanFilesModal + their dead CSS. Back-compat: legacy projects (no `generatedAlternatives`) open to the wizard. Browser-verified end-to-end, zero console errors. Studio gut sequenced bottom-up (Studio→App→Results) so the build stayed green.
- **Vitest seeded** (post-B): `sceneGestures.ts` pure rotate-decision + `sceneGestures.test.ts` (locks the rotate-grip regression), `SceneEditor.test.tsx` (DOM rotate + delete-gizmo propagation, **mutation-verified** — removing the fix fails the test). `npm test` = 8 pass. Standing practice now: mutation-verify every regression test.
- **Workstream C DONE (program anchor pins).** `DetailedProgram.anchors: [{room_type,x,y}]` (plan feet). Placer seats anchored rooms **hard, first, deterministically** (largest-first, coord tiebreak) via `rooms.place_at_anchor` (containing the point), before free placement; unsatisfiable/competing anchors → per-variant `unsatisfied_anchors` (honest) + `program_match` penalty (unplaced instance, never backfilled). UI: Program-step type-picker arms a pin, click the plan to drop it (reuses the marker rendering), panel pin-list to delete; **count-coupling** (a pin bumps its type's count, delete never decrements); anchors **cleared on plate re-upload** (stale coords would mis-seed a new building). Coordinate frames verified aligned (read-layout ↔ generation PlanModel). 4 backend tests incl. the competition case; also fixed a latent `_placed_by_catalog_key` crash the impossible-anchor case exposed. **Folded in B's descope:** `sceneProgramTargets` persisted onto `generatedAlternatives`, so a forked design's editor scoreboard shows actual-vs-target again. Browser-verified end-to-end.
- **Next: D (N-candidate generation culled by the scorer)** — widen to 10–16 seeds, cull to top 4–5 via `score_variants`, anchors held fixed. Then E (agent-critic, offline/gated). Guardrails: if wall-clock > a few seconds on the fixture, trim the pool (16→10) before building progressive return; if two fixture programs can't yield different recommended variants, that's a weights finding — stop and report, don't ship.
